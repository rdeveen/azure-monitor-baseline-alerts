...
> [2026-06-03](#2026-06-03) </br>
> [2026-03-06](#2026-03-06) </br>
...
- Ability to filter alerts notified or suppressed by the AMBA-ALZ Alert Processing Rule to only include those generated by AMBA-ALZ alerts.
- Email notifications now have the _**"Enable the common alert schema"**_ enabled.
- Added alerts for Azure Virtual Machine Scale Set.
- Log alerts have been optimize to use the _**lookup**_ operator intstead of _**join**_

### Bug Fixes

- Fixed [[#907](https://github.com/Azure/azure-monitor-baseline-alerts/issues/907)]: [General workload issue]: Start-AMBA-ALZ-Maintenance - cannot bind argument to parameter 'ManagementGroup' because it is an empty array
- Fixed [[#894](https://github.com/Azure/azure-monitor-baseline-alerts/issues/894)]: [General workload issue]: Link to AVD monitors is broken
- Fixed [[#879](https://github.com/Azure/azure-monitor-baseline-alerts/pull/879)]: Bug: Deploy to Azure button was not working
- Fixed [[#845](https://github.com/Azure/azure-monitor-baseline-alerts/issues/845)]: [General workload issue]: Alert Processing Rule picks up non-AMBA alerts, causing unwanted notifications to platform team
- Fixed [[#749](https://github.com/Azure/azure-monitor-baseline-alerts/issues/749)]: [General workload issue]: Log search queries failing to execute on Heartbeat fail
...
- The maintenance script has been updated to:
  - resolve an off-by-one bug in the batching loop that triggers exactly when MG count is a multiple of 10.
  - retrieve AMBA-ALZ deployment which are now using a new naming convention starting with _*amba-alz-*_.
  - include a break statement in the switch body for each case to avoid the default fall-through behavior
...
- New documentation theme allowing for a better readability and user experience.
